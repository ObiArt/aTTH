    ...
    public class Entity
    {
        ...
        public virtual void CollissionCheck(List<Entity> entities)
        {
            for (int i = 0; i < entities.Count; i++)
            {
                ...
                {
                    if ((Math.Abs(position.Y - entities[i].position.Y) < (origin.Y + entities[i].origin.Y) * Params._scale) 
                        & (Math.Abs(position.X - entities[i].position.X) < (origin.X + entities[i].origin.X) * Params._scale))
                    {
                        ...
                        int smallest = 0;
                        float diff = Math.Abs(position.Y - dirs[0]);
                        for (int u = 1; u < dirs.Count; u++) //no reason to check the first one
                        {
                            if (u == 1)
                            {
                                if (Math.Abs(position.Y - dirs[u]) < diff)
                                {
                                    smallest = u;
                                    diff = Math.Abs(position.Y - dirs[u]);
                                }
                            } else
                            {
                                if (Math.Abs(position.X - dirs[u]) < diff)
                                {
                                    smallest = u;
                                    diff = Math.Abs(position.X - dirs[u]);
                                }
                            }
                        }
